...
void run_model(void) {
  ...
  while (sol_continue()) {
    ...
    case 13: {
      ...
      sol_uint256_t prev_send = Read_Map_1(&contract_0.user___balances, arg_from);
      sol_uint256_t prev_dest = Read_Map_1(&contract_0.user___balances, arg_to);
      ERC20_Method_For_Melon_transferFrom(&(contract_0), sender, value,
                                          blocknum, timestamp, paid, sender,
                                          arg_from, arg_to, arg_value);
      if (arg_from.v != arg_to.v)
      {
        sol_uint256_t post_send = Read_Map_1(&contract_0.user___balances, arg_from);
        sol_uint256_t post_dest = Read_Map_1(&contract_0.user___balances, arg_to);
