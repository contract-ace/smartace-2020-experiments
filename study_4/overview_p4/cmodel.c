...
void run_model(void) {
  ...
  while (sol_continue()) {
    ...
    case 4: {
      ...
      // In VerX specs FUNCTION is top level: this is sufficient.
      prev_sum = contract_1->user_deposits.sum;
      prev_goal = contract_0.user_goal;
      Escrow_Method_claimRefund(contract_1, sender, value, blocknum, timestamp,
                                paid, sender, arg_p);
      post_sum = contract_1->user_deposits.sum;
      post_goal = contract_0.user_goal;
