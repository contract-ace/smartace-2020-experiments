...
void run_model(void) {
  ...
  while (sol_continue()) {
    ...
    last = contract_0.user_maxCrowdsaleSupplyInWholeTokens;
    switch (next_call) {
    case 0: {
    ...
    if (contract_0.user_crowdsaleStarted.v != 0)
    {
      sol_assert(last.v == contract_0.user_maxCrowdsaleSupplyInWholeTokens.v, 0);
    }
  }
}