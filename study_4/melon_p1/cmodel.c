...
void run_model(void) {
  ...
  while (sol_continue()) {
    ...
    prev_bal = Read_Map_1(&contract_0.user___balances, watched);
    switch (next_call) {
    case 0: {
    ...
    post_bal = Read_Map_1(&contract_0.user___balances, watched);
    if (prev_bal.v > post_bal.v)
    {
      sol_assert(curr_sender.v == watched.v || call_1  || call_2 || call_3, 0);
    }
  }
}